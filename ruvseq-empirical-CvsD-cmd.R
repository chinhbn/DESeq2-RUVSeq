...
dim(D)
ix.1<-grep("_C_",names(D))
ix.2<-grep("_D_",names(D))
...
for (l in 1:L){
	...
	#col2<-as.matrix(col2)
	dds2 <- DESeqDataSetFromMatrix(countData = X,colData = col2,design=~condition)
...
pdf("SEQC-RUVSEQempirical-DESEQ2-CvsD-k1-10ercc-ROC-curves.pdf")
plot(roc.res.1)
